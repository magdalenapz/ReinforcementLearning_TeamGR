...
class TD3_PER(object):
    def __init__(
        ...
        noise_clip=0.5,
        policy_freq=2,
        # extended parameterlist for PER
        device="cpu",
        alpha=0.4,
        beta=0.4,
    ...
    def train(self, replay_buffer, batch_size=256, beta=None):
        self.total_it += 1

        # beta for PER, is used if provided, otherwise use initialized
        beta_used = self.beta if beta is None else beta
		# Sample replay buffer 
        state, action, next_state, reward, not_done, ind, weights = replay_buffer.sample(batch_size, beta_used)
        ...
        critic_loss = (weights * (F.mse_loss(current_Q1, target_Q, reduction='none') + F.mse_loss(current_Q2, target_Q, reduction='none'))).mean()
        ...
        # TD errors to update priorities in PER
        ...
        replay_buffer.update_priorities(ind, td_errors)
